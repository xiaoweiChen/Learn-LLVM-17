只是缺少消息的集中定义。在大型软件(如编译器)中，应该不希望到处散布消息字符串。若有修改消息或将其翻译成另一种语言的请求，最好将它们放在一个中心位置!一种简单的方法是，每个消息都有一个ID(枚举成员)、一个严重级别(如Error或Warning)和一个包含消息的字符串。在代码中，只需要引用消息ID即可。
