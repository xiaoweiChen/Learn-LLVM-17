...
\item
现在，可以实现第一个调用事件方法checkPostCall()。此方法在分析器执行函数调用后调用。若执行的函数不是一个全局C函数，也没有命名为iconv\_open，就没什么可做的了:
...
\item
否则，可以尝试以符号的形式获取函数的返回值。为了在全局程序状态中存储具有打开状态的符号，需要从CheckerContext实例中获取一个ProgramStateRef实例。状态是不可变的，因此将符号添加到状态会产生一个新状态。最后，分析器引擎通过调用addTransition()方法获知新状态:
...
\item
若函数的第一个参数的符号(即iconv描述符)是已知的，则可以从程序状态中检索该符号的状态:
